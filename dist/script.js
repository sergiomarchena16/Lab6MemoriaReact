  ...
  render() {
    ...
    return (
      ...
      this.state.finalizedFrameworks.map((framework, index) => {
        return React.createElement(Card, { framework: framework.name, click: () => {this.handleClick(framework.name, index);}, close: framework.close, complete: framework.complete });
      })));
...
class Card extends React.Component {
  ...
  render() {
    return (
      React.createElement("div", { className: "card" + (!this.props.close ? ' opened' : '') + (this.props.complete ? ' matched' : ''), onClick: () => this.clicked(this.props.framework) },
